...
function recursivelyApplyOptions(
  watch: string[],
...
function resolveSingle(
  ...
  switch (script.kind) {
    ...
    case "script-reference": {
      ...
      return recursivelyApplyOptions(
        script.watch ?? [],
        script.onExit,
        resolved,
      );
    ...
    case "script-sequence": {
      const resolvedScripts = script.scripts
        ...
        .map((s) =>
          recursivelyApplyOptions(script.watch ?? [], script.onExit, s),
        );
...
function resolveScript(
  ...
  if (scriptName.includes("->")) {
    ...
    return resolveSingle(foundScript, scripts);
  }
...
export const resolveAndExecuteCommand = async (
  command: string,
  args: string[],
  scripts: WorkspaceScripts,
  locator: Locator,
  ...
  const commands = asArray
    ...
    .reduce(
      (acc, x) => [...acc, ...x],
      ["_r_internal_1", "--args", JSON.stringify(args)],
    );

  scriptUtils.executePackageShellcode(locator, "yarn", commands, {
    project,
    ...stdio,
  });
};
